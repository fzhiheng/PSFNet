...
class PointNetSaModule(nn.Module):
    ...
    def forward(self, xyz, xyz_raw, label, points, sample_idx=None):
        ...
        if xyz_raw is not None:
            ...
            # new_xyz 和 new_xyz_raw 都是通过相同的索引分别对xyz和xyz_raw采样得到的
            new_xyz, new_label, new_points, new_xyz_raw, sample_idx = sample_and_group(sample_idx, self.npoint,self.radius,
        ...
        new_points = new_points.squeeze(
            2)  # (batch_size,npoint, mlp2[-1]) if mlp2 is not None else  (batch_size,npoint, mlp[-1])
...
class SceneFlow_Estimator(nn.Module):
    ...
    def __init__(self, in_channels, flow_ch=3, occ_ch=1, channels=[128, 128], mlp=[128, 64], neighbors=9, clamp=[-200, 200],
                 use_leaky=True):
...
class FlowPredictor(nn.Module):
    ...
    def forward(self, points_f1: torch.Tensor, upsampled_feat: torch.Tensor, cost_volume: torch.Tensor, occ_mask = None) -> torch.Tensor:
        """
...
class PointConv(nn.Module):
    ...
    def forward(self, xyz, points):
        ...
        new_points = torch.matmul(input=new_points.permute(0, 1, 3, 2), other=weights.permute(0, 3, 2, 1))#  [B, N, C+D, weightnet] = [B, N, C+D, nsample]*[B, N, nsample, weightnet]
