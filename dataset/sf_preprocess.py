...
# @Time : 2022/10/17 12:00
# @Author : Zhiheng Feng
# @File : sf_preprocess.py
# @Software : PyCharm
...
class RangeLimitWithSF(BaseRangeLimit):
    def __init__(self, *, x_range=(-30, 30), y_range=(-1, 1.4), z_range=(0, 35), use_same_indice=False):
        """
        x_range,y_range,z_range 分别表示三个维度的限制范围，闭区间
        Args:
            x_range:
            y_range:
            z_range:
            use_same_indice: 两帧点云是否使用相同的切片
        """
...
class TransformerWithSF(BaseTransform):
    ...
    def __call__(self, data_dict):
        pc1 = self.transformer_tr(data_dict['pc1'], self.Tr)
        pc2 = self.transformer_tr(data_dict['pc2'], self.Tr)
        flow = self.transformer_tr(data_dict['flow'], self.Tr)
        data_dict['pc1'] = pc1
        data_dict['pc1'] = pc2
        data_dict['flow'] = flow
        return data_dict
